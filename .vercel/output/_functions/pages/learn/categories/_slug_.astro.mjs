...
import { $ as $$Cta } from '../../../chunks/Cta_DuBobZTO.mjs';
...
async function getStaticPaths() {
  ...
  return categories.map((category) => ({
    ...
    props: { category, allArticles: articles }
  }));
}
const $$slug = createComponent(async ($$result, $$props, $$slots) => {
  ...
  const allCategories = [
    ...new Set(allArticles.flatMap(
      (article) => normalizeToArray2(article.data.categories)
    ))
  ];
  ...
  const filteredArticles = allArticles.filter((article) => {
    const categoriesArray = normalizeToArray2(article.data.categories).map((c) => c.trim().toLowerCase());
